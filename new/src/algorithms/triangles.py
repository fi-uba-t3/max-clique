...
class Triangles():
    ...
    def _pop_element(self):
        ...
        self.nodes_per_triangles = list(map(lambda x: x - 1, self.nodes_per_triangles))
        ## Remove empty degrees list
        while self.nodes_per_triangles and self.nodes_per_triangles[0] == 0:
            self.nodes_per_triangles.pop(0)
    ...
    def _get_max_clique_size(self, degree):
        ...
        nodes_per_triangles_sliced = self.nodes_per_triangles[- degree - 1:]
        
        for amount_of_nodes in nodes_per_triangles_sliced:
    ...
    def get_t_n_iterator(self):
        ...
        self.nodes_per_triangles = []
        self.T.reverse()
        previous_value = 0
        
        for list_of_nodes in self.T:
            self.nodes_per_triangles.append(len(list_of_nodes) + previous_value)
            previous_value = self.nodes_per_triangles[-1]
